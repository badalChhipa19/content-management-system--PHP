    ...
    switch ($option) {
      ...
      case 'clone':
        ...
        while ($row = mysqli_fetch_assoc($result_for_clone)) {
          ...
          $post_user_id     = escape($row['post_user_id']);
        ...
        $query_for_cloning  = "INSERT INTO posts(post_author, post_title, post_cat_id, post_status, post_image, post_tags, post_date, post_content, post_user_id)
                                VALUES ('$clone_author', '$clone_title', '$clone_cat_id', '$clone_status', '$clone_img', '$clone_tags', '$clone_date', '$post_content', '$post_user_id')";
...
<form action="" method="post">
  ...
  <table class="table table-bordered table-hover">
    ...
    <tbody>
      ...
      // $query = "SELECT post.post_id, post.post_author, post.post_title, post.post_cat_id, post.post_status, post.post_image, post.post_tags, post.post_date, post.post_view_count,
      //                 cat.cat_id, cat.cat_title FROM posts LEFT JOIN cat ON post.post_cat_id = cat.cat_id ORDER BY post.post_id DESC";
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<td><a class='btn btn-danger' onclick=\"javascript: return confirm('Are you sure you want to delete this post')\" href='posts.php?delete={$post_id}'>Delete</td>";
        echo "</tr>";
      }
