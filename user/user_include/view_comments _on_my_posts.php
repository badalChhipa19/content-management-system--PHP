    ...
    switch ($option) {
      ...
      case 'clone':
        ...
        while ($row = mysqli_fetch_assoc($result_for_clone)) {
          $comm_user_id   = escape($row['comm_user_id']);
        ...
        $query_for_cloning  = "INSERT INTO comments(comm_post_id, comm_author, comm_mail, comm_content, comm_status, comm_date, comm_user_id) 
                                VALUES('$comm_post_id', '$caomm_author', '$mail', '$comm_content', 'unapproved', now(), $comm_user_id)";
    ...
    <tbody>
      ...
      while ($fetched_posts = mysqli_fetch_array($result_fetch_posts)) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
          ...
          echo "<td>$date</td>";
          echo "<td><a href='my_comments.php?approve=$comm_id''>Approve</a></td>";
          echo "<td><a href='my_comments.php?unapprove=$comm_id''>Unapprove</a></td>";
          echo "<td><a class='btn btn-danger' onclick=\"javascript: return confirm('Are you sure you want to delete this comment')\" href='my_comments.php?delete=$comm_id'>Delete</a></td>";
          echo "</tr>";
        }
      }
